...
public class Katana_3Set : MonoBehaviour {
	...
	//list store numbers
	public List<int> katana_3Set1 = new List<int>();
	//depending on the values in set1, generate corresponding elements in set2
	public List<GameObject> katana_3Set2 = new List<GameObject> ();
	...
	void Start () {
		...
		*/
		...
		for (int i = 0; i < sizeOfSet; i++) {
			...
			katana_3Set2[i] = (GameObject) Instantiate (katana_3Set2[i],new Vector3 (i,i,0),Quaternion.Euler(new Vector3(45, 0, 0)));
			...
			katana_3Set2[i].transform.position = pos;
			katana_3Set2[i].transform.parent = center.transform;
	...
	void Update () {
		...
		if(UnionAction.katana3isDestroyed == 1){
			//Debug.Log("katana 2 is destroyed");
			for (int i = 0; i < sizeOfSet; i++)
			{
				katana_3Set2[i].gameObject.SetActive(false);
			}
		}
	}
}