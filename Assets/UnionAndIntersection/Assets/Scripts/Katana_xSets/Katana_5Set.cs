...
public class Katana_5Set : MonoBehaviour {
	...
	//list store numbers
	public List<int> katana_5Set1 = new List<int>();
	//depending on the values in set1, generate corresponding elements in set2
	public List<GameObject> katana_5Set2 = new List<GameObject> ();
	...
	void Start () {
		...
		*/
		...
		for (int i = 0; i < sizeOfSet; i++) {
			...
			katana_5Set2[i] = (GameObject) Instantiate (katana_5Set2[i],new Vector3 (i,i,0),Quaternion.Euler(new Vector3(45, 0, 0)));
			...
			katana_5Set2[i].transform.position = pos;
			katana_5Set2[i].transform.parent = center.transform;
	...
	void Update () {
		...
		if(UnionAction.katana5isDestroyed == 1){
			//Debug.Log("katana 2 is destroyed");
			for (int i = 0; i < sizeOfSet; i++)
			{
				katana_5Set2[i].gameObject.SetActive(false);
			}
		}
	}
}
