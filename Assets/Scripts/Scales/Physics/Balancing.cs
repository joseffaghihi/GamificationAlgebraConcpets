...
public class Balancing : MonoBehaviour
{
	...
	void Start () 
    {
        //*Audio*
        //-------
		audioManager = new AudioManager("Scales/MyAudio");
	...
	void FixedUpdate() //Frame Independent Loop 
    {
        if (physics)
        {
            ...
            if (int.Parse(userInput.text) > equation.GetSolution())
            {
                //*Physics*
				//---------
                rigidbody.AddRelativeTorque(rotationSpeed, 0, 0);
                ...
                if (!addedStat)
                {
                    //*Audio*
                    //-------
                    audio.PlayOneShot(audioManager.GetClip("Shriek"));
			...
			else if (int.Parse(userInput.text) == equation.GetSolution())
            {
                ...
                {
                    audio.PlayOneShot(audioManager.GetClip("ProblemSolved")); //Victory Sound
    ...
    void playAnimation(int num, string animation)
    {
        ...
        littleGuys[num].animation.Play(animation);
    }
